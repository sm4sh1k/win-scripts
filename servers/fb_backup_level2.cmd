...
::Script for making incremental Firebird database backup (level 2) via nbackup utility,
:: archiving it with 7-zip and deleting old archives (7 days older).
...
SET archivename=MAIN_2_%year%-%month%-%day%

ECHO Creating database backup (level 2)...
%nbackup% -U %dbuser% -P %dbpass% -B 2 %database% %archivepath%%archivename%.nbk
...
ECHO Deleting old archives (7 days older)...
CD /D "%archivepath%"
FORFILES /M MAIN_2_*-*-*.7z /D -7 /C "cmd /c if @isdir==FALSE del /f /q @file"
EXIT 0