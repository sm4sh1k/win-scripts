...
::Script for making full Firebird database backup (level 0) via nbackup utility,
:: archiving it with 7-zip and deleting old archives (95 days older).
...
SET archivename=MAIN_0_%year%-%month%-%day%

ECHO Creating database backup (level 0)...
%nbackup% -U %dbuser% -P %dbpass% -B 0 %database% %archivepath%%archivename%.nbk
...
ECHO Deleting old archives (95 days older)...
CD /D "%archivepath%"
FORFILES /M MAIN_0_*-*-*.7z /D -95 /C "cmd /c if @isdir==FALSE del /f /q @file"
EXIT 0
